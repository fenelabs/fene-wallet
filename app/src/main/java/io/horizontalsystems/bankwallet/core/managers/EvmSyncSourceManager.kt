...
class EvmSyncSourceManager(
    ...
    private fun defaultTransactionSource(blockchainType: BlockchainType): TransactionSource {
        return when (blockchainType) {
            ...
            BlockchainType.Davinci -> TransactionSource.davinci(appConfigProvider.davinciscanApiKey)
    ...
    fun defaultSyncSources(blockchainType: BlockchainType): List<EvmSyncSource> {
        return when (blockchainType) {
            ...
            BlockchainType.Davinci -> listOf(
                evmSyncSource(
                    blockchainType,
                    "Davinci Chain",
                    RpcSource.Http(listOf(URI("https://rpc.davinci.bz")), null),
                    defaultTransactionSource(blockchainType)
                ),
                evmSyncSource(
                    blockchainType,
                    "Davinci Checkpoint",
                    RpcSource.Http(listOf(URI("https://rpc-checkpoint.davinci.bz")), null),
                    defaultTransactionSource(blockchainType)
                )
            )
